...
import io.github.cactacea.filhouette.api.crypto.{Base64, Signer}
...
object SocialStateItem {
   ...
   */
  case class ItemStructure(id: String, data: JsonNode) {
...
trait SocialStateHandler {
   ...
   */
  def unserialize(state: String)(implicit request: Request): Future[SocialState]
   ...
   */
  def publish(result: Response, state: SocialState)(implicit request: Request): Response
}
...
class DefaultSocialStateHandler(val handlers: Set[SocialStateItemHandler], signer: Signer)
   ...
   * @return The social state for all handlers.
   */
  override def state(): Future[SocialState] = {
    Future.collect(handlers.map(_.item).toSeq).map(items => SocialState(items.toSet))
  }
   ...
   * @return The social state on success, an error on failure.
   */
  override def unserialize(state: String)(
    implicit
    request: Request
  ): Future[SocialState] = {
    if (handlers.isEmpty) {
      ...
      Future.const(signer.extract(state)).flatMap { state =>
        state.split('.').toList match {
          case Nil | List("") =>
            Future.value(SocialState(Set()))
          case items =>
            Future.traverseSequentially(items) {
              _ match {
   ...
   */
  override def publish(result: Response, state: SocialState)(implicit request: Request): Response = {
...
trait SocialStateItemHandler {
   ...
   */
  def canHandle(item: ItemStructure)(implicit request: Request): Boolean
   ...
   */
  def unserialize(item: ItemStructure)(
    implicit
...
trait PublishableSocialStateItemHandler {
   ...
   * @return The result to send to the client.
   */
  def publish(item: Item, result: Response)(implicit request: Request): Response
}
