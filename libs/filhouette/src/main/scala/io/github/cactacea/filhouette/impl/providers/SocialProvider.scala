...
import io.github.cactacea.filhouette.api.{AuthInfo, LoginInfo, Provider}
import io.github.cactacea.filhouette.impl.exceptions.ProfileRetrievalException
import io.github.cactacea.filhouette.impl.providers.SocialProfileBuilder._
...
trait SocialProvider extends Provider with SocialProfileBuilder {
  ...
  def retrieveProfile(authInfo: A): Future[Profile] = {
    buildProfile(authInfo).rescue {
      case e if !e.isInstanceOf[ProfileRetrievalException] =>
        Future.exception(new ProfileRetrievalException(UnspecifiedProfileError.format(id), e))
    }
  }
  ...
  /**
    ...
    */
  protected def resolveCallbackURL(url: String)(implicit request: Request): String = URI.create(url) match {
    case uri if uri.isAbsolute => url
    case uri =>
      val scheme = "https://" // if (request.secure) "https://" else "http://"
...
object SocialProfileBuilder {
  ...
  /**
    * Some error messages.
    */
  val UnspecifiedProfileError = "[Filhouette][%s] error retrieving profile information"
}
