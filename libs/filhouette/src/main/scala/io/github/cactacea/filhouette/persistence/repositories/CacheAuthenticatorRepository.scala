...
import io.github.cactacea.filhouette.api.StorableAuthenticator
import io.github.cactacea.filhouette.api.repositories.AuthenticatorRepository
import io.github.cactacea.filhouette.api.util.CacheLayer
...
class CacheAuthenticatorRepository[T <: StorableAuthenticator: ClassTag] @Inject()(cacheLayer: CacheLayer)
   ...
   */
  override def add(authenticator: T): Future[T] = cacheLayer.save[T](authenticator.id, authenticator, Duration.Top)
   ...
   */
  override def update(authenticator: T): Future[T] = cacheLayer.save[T](authenticator.id, authenticator, Duration.Top)
