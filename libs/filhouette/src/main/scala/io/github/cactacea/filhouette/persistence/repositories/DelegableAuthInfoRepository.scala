/**
  ...
  */
package io.github.cactacea.filhouette.persistence.repositories
...
import io.github.cactacea.filhouette.persistence.daos.{AuthInfoDAO, DelegableAuthInfoDAO}
import io.github.cactacea.filhouette.persistence.repositories.DelegableAuthInfoRepository._
...
class DelegableAuthInfoRepository(daos: DelegableAuthInfoDAO[_]*) extends AuthInfoRepository {
  ...
  override def find[T <: AuthInfo](loginInfo: LoginInfo)(implicit tag: ClassTag[T]): Future[Option[T]] = {
    daos.find(_.classTag == tag) match {
      ...
      case _         => Future.exception(new ConfigurationException(FindError.format(tag.runtimeClass)))
    }
  }
  ...
  override def add[T <: AuthInfo](loginInfo: LoginInfo, authInfo: T): Future[T] = {
    daos.find(_.classTag.runtimeClass == authInfo.getClass) match {
      ...
      case _         => Future.exception(new ConfigurationException(AddError.format(authInfo.getClass)))
    }
  }
  ...
  override def update[T <: AuthInfo](loginInfo: LoginInfo, authInfo: T): Future[T] = {
    daos.find(_.classTag.runtimeClass == authInfo.getClass) match {
      ...
      case _         => Future.exception(new ConfigurationException(UpdateError.format(authInfo.getClass)))
    }
  }
  ...
  override def save[T <: AuthInfo](loginInfo: LoginInfo, authInfo: T): Future[T] = {
    daos.find(_.classTag.runtimeClass == authInfo.getClass) match {
      ...
      case _         => Future.exception(new ConfigurationException(SaveError.format(authInfo.getClass)))
    }
  }
  ...
  override def remove[T <: AuthInfo](loginInfo: LoginInfo)(implicit tag: ClassTag[T]): Future[Unit] = {
    daos.find(_.classTag == tag) match {
      ...
      case _         => Future.exception(new ConfigurationException(RemoveError.format(tag.runtimeClass)))
    }
  }
}
