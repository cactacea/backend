...
class WebjarsController @Inject()(resolver: FileResolver) extends Controller {
  ...
  get(s"${root}/:*") { request: Request =>
    ...
    val overridePath = "webjar_overrides/" + resourcePath
        ...
        case Success(eTagName) =>
          if (!disableCache) {
            if (checkETagMatch(request, eTagName) || checkLastModify(request)) {
              ...
              val inputStream =
                Option(getClass.getClassLoader.getResourceAsStream(overridePath)).
                  getOrElse(getClass.getResourceAsStream(webjarsResourceURI))
              if (inputStream != null) {
