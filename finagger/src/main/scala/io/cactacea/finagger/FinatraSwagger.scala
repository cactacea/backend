...
class FinatraSwagger(swagger: Swagger) {
      ...
      properties.collect {
        ...
        case param @ (_: RouteRequestParam) =>
          new PathParameter().
            ...
            property(registerProperty(param.typ, param.parameterizedType.getOrElse(param.typ)))
  ...
  private def getFinatraProps[T: TypeTag]: List[FinatraRequestParam] = {
    ...
    val ast: List[Option[FinatraRequestParam]] =
      constructorArgWithField.map { case (annotationExtractor, field) =>
        ...
        val (name, description, annotations, hidden) = modelProp match {
          case Some(p) =>
            val n = if(!p.name().isEmpty) p.name() else {
              field.getName
            }
            (n, p.value(), modelPropertyAnnotations.map(_.toList).getOrElse(Nil), p.hidden())
          case None =>
            (field.getName, "", field.getAnnotations.toList, false)
        }
  ...
  def registerProperty[T: TypeTag](example: Option[T]): Property = {
  ...
  def registerModel[T: TypeTag](example: Option[T]): Option[Model] = {
  ...
  private def registerProperty(typeConstructor: java.lang.reflect.Type, typ: java.lang.reflect.Type, example: Option[_] = None): Property = {
    ...
    for (entry <- models.entrySet().asScala) {
      ...
      example.foreach(model.setExample)
