...
import io.github.cactacea.core.application.components.modules.DatabaseProviderModule
  ...
  def initialize() = {
    ...
    val r = db.transaction {
      for {
        ...
        _ <- db.run(quote(query[CommentLikes].delete))
        _ <- db.run(quote(query[FeedLikes].delete))
