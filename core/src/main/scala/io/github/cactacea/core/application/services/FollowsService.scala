...
@Singleton
class FollowsService @Inject()(db: DatabaseService, followsRepository: FollowsRepository, actionService: InjectionService) {

  def create(accountId: AccountId, sessionId: SessionId): Future[Unit] = {
    for {
      r <- db.transaction(followsRepository.create(accountId, sessionId))
  ...
  def delete(accountId: AccountId, sessionId: SessionId): Future[Unit] = {
    for {
      r <- db.transaction(followsRepository.delete(accountId, sessionId))
