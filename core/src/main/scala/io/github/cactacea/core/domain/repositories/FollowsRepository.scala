...
import io.github.cactacea.core.infrastructure.dao.{FollowersDAO, FollowsDAO, GroupInvitesDAO, ValidationDAO}
...
class FollowsRepository {
  ...
  def delete(accountId: AccountId, sessionId: SessionId): Future[Unit] = {
    for {
      ...
      _ <- groupInvitesDAO.delete(accountId, GroupPrivacyType.follows, sessionId)
      _ <- groupInvitesDAO.delete(sessionId.toAccountId, GroupPrivacyType.followers, accountId.toSessionId)
    } yield (Future.value(Unit))
  }
