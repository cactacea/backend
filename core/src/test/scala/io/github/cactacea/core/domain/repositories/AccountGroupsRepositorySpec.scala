...
class AccountGroupsRepositorySpec extends RepositorySpec {
  ...
  test("delete") {
    ...
    val result1 = execute(accountGroupsRepository.findAll(None, 0, 20, false, sessionUser.id.toSessionId))
    assert(result1.size == 0)
    val result2 = execute(messagesRepository.findAll(groupId, None, 0, 20, true, sessionUser.id.toSessionId))
  ...
  test("find all user groups") {
    ...
    val groupId3 = execute(groupsRepository.create(Some("group name 3"), true, GroupPrivacyType.following, GroupAuthorityType.member, user.id.toSessionId))
    ...
    val result = execute(accountGroupsRepository.findAll(user.id, None, 0, 20, sessionUser.id.toSessionId))
  ...
  test("find session's user groups") {
    ...
    val groupId3 = execute(groupsRepository.create(Some("group name 3"), true, GroupPrivacyType.following, GroupAuthorityType.member, sessionUser.id.toSessionId))
    ...
    val result = execute(accountGroupsRepository.findAll(None, 0, 20, false, sessionUser.id.toSessionId))
    assert(result.size == 5)
    assert(result(0).id == groupId5)
    assert(result(1).id == groupId4)
    assert(result(2).id == groupId3)
  ...
  test("create one to one group") {
    ...
    val result1 = execute(accountGroupsRepository.findAll(None, 0, 20, false, sessionUser.id.toSessionId))
    val result2 = execute(accountGroupsRepository.findAll(None, 0, 20, false, user.id.toSessionId))
  ...
  test("show") {
    ...
    val result2 = execute(accountGroupsRepository.findAll(None, 0, 20, false, sessionUser.id.toSessionId))
