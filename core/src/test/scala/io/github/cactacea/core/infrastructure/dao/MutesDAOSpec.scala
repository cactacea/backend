...
class MutesDAOSpec extends DAOSpec {
  ...
  test("findAll") {
    ...
    val result1 = execute(mutesDAO.findAll(None, None, Some(3), muteUser.id.toSessionId))
    ...
    val result2 = execute(mutesDAO.findAll(Some(result1(2)._3.id.value), None, Some(3), muteUser.id.toSessionId))
