...
class MediumsDAOSpec extends DAOSpec {
  ...
  test("create") {
    ...
    assert((medium1.id, medium1.by, medium1.width, medium1.height, medium1.size, medium1.key, medium1.mediumType, medium1.uri, medium1.thumbnailUri) == (mediumId1, sessionAccount.id, 1, 4, 100L, "key1", MediumType.image.toValue, "http://example.com/test1.jpeg", Some("http://example.com/test1.jpeg")))
    assert((medium2.id, medium2.by, medium2.width, medium2.height, medium2.size, medium2.key, medium2.mediumType, medium2.uri, medium2.thumbnailUri) == (mediumId2, sessionAccount.id, 2, 5, 200L, "key2", MediumType.movie.toValue, "http://example.com/test2.mov", Some("http://example.com/test2.mov")))
    assert((medium3.id, medium3.by, medium3.width, medium3.height, medium3.size, medium3.key, medium3.mediumType, medium3.uri, medium3.thumbnailUri) == (mediumId3, sessionAccount.id, 3, 6, 300L, "key3", MediumType.image.toValue, "http://example.com/test3.jpeg", Some("http://example.com/test3.jpeg")))
