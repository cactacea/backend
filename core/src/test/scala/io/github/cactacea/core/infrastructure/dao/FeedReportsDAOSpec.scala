...
class FeedReportsDAOSpec extends DAOSpec {
  ...
  test("create") {
    ...
    execute(feedReportsDAO.create(feedId, ReportType.inappropriate, reportContent, sessionAccount2.id.toSessionId))
    execute(feedReportsDAO.create(feedId, ReportType.inappropriate, reportContent, sessionAccount3.id.toSessionId))
