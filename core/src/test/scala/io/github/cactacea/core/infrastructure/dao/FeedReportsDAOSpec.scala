...
class FeedReportsDAOSpec extends DAOSpec {
  ...
  test("create") {
    ...
    val reportId1 = Await.result(feedReportsDAO.create(feedId, ReportType.inappropriate, reportContent, sessionAccount2.id.toSessionId))
    val reportId2 = Await.result(feedReportsDAO.create(feedId, ReportType.inappropriate, reportContent, sessionAccount3.id.toSessionId))
