...
class GroupInvitationsRepositorySpec extends RepositorySpec {
  ...
  test("invitation accounts to a groups") {
    ...
    val result1 = execute(groupInvitationsRepository.findAll(None, 0, 20, user1.id.toSessionId))
    val result2 = execute(groupInvitationsRepository.findAll(None, 0, 20, user2.id.toSessionId))
    val result3 = execute(groupInvitationsRepository.findAll(None, 0, 20, user3.id.toSessionId))
    val result4 = execute(groupInvitationsRepository.findAll(None, 0, 20, user4.id.toSessionId))
    val result5 = execute(groupInvitationsRepository.findAll(None, 0, 20, user5.id.toSessionId))
