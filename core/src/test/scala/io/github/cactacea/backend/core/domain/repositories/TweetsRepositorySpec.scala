...
import io.github.cactacea.backend.core.domain.enums.{TweetType, NotificationType}
  ...
  feature("create") {
    ...
    scenario("should create an user tweet and a notification") {
      forOne(userGen, userGen, followerTweetGen) { (s, a, f) =>
        ...
        assert(result.headOption.exists(_.notificationType == NotificationType.tweet))
      }
    }
