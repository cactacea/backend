...
import io.github.cactacea.backend.utils.auth.{CactaceaAccount, CactaceaContext}
...
class CactaceaAuthenticationFilter @Inject()(accountsRepository: AccountsRepository,
                                             securedAction: SecuredActionBuilder[CactaceaAccount, JWTAuthenticator]
                                  ) extends SimpleFilter[Request, Response] {

  override def apply(request: Request, service: Service[Request, Response]): Future[Response] = {
    val block = (securedRequest: SecuredRequest[CactaceaAccount, JWTAuthenticator]) => {
