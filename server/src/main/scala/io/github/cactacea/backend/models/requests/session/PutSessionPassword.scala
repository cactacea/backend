...
case class PutSessionPassword(
  ...
  @MethodValidation
  def oldPasswordCheck: ValidationResult = CactaceaValidations.validatePassword(oldPassword)

  @MethodValidation
  def newPasswordCheck: ValidationResult = CactaceaValidations.validatePassword(newPassword)
