...
import io.github.cactacea.filhouette.impl.crypto.JcaSigner._
...
class JcaSigner(settings: JcaSignerSettings) extends Signer {
  ...
  private def fragment(message: String): Try[(String, String, String)] = {
    message.split("-", 3) match {
      case Array(version, signature, data) if version == "1" => Return((version, signature, data))
      case Array(version, _, _) => Throw(new CryptoException(UnknownVersion.format(version)))
      case _ => Throw(new CryptoException(InvalidMessageFormat))
    }
  }
